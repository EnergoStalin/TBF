...
tbf::RenderFix::tbf_draw_states_s tbf::RenderFix::draw_state;

aspect_ratio_s aspect_ratio;

struct smaa_constants_s {
  struct {
    float inv_x, inv_y, width, height;
  } metrics;

  struct {
    float threshold, max_search_steps, max_search_steps_diag, corner_rounding;
  } general;
...
uint32_t TBF_MakeShadowBitShift(uint32_t dim) {
...
void TBF_ComputeAspectCoeffs(float& x, float& y, float& xoff, float& yoff) {
  yoff = 0.0f;
  xoff = 0.0f;
...
HRESULT
STDMETHODCALLTYPE
D3D9SetSamplerState_Detour(IDirect3DDevice9* This, DWORD Sampler,
  ...
  if (Type == D3DSAMP_MIPMAPLODBIAS) {
    ...
    if (Type == D3DSAMP_MIPMAPLODBIAS) {
      float fMax =
          config.fun_stuff.plastic_mode ? 20.0f : config.textures.lod_bias;
...
void SK_D3D9_DumpShader(const wchar_t* wszPrefix, uint32_t crc32,
                        LPVOID pbFunc) {
  ...
  if (D3DXDisassembleShader != nullptr) {
    if (dump) {
      ...
      swprintf_s(wszDumpName, MAX_PATH,
                 L"TBFix_Res\\dump\\shaders\\%s_%08x.html", wszPrefix, crc32);

      if (GetFileAttributes(wszDumpName) == INVALID_FILE_ATTRIBUTES) {
        CComPtr<ID3DXBuffer> pDisasm = nullptr;

        HRESULT hr = D3DXDisassembleShader((DWORD*)pbFunc, TRUE, "", &pDisasm);

        if (SUCCEEDED(hr)) {
          FILE* fDump = _wfsopen(wszDumpName, L"wb", _SH_DENYWR);

          if (fDump != NULL) {
            fwrite(pDisasm->GetBufferPointer(), pDisasm->GetBufferSize(), 1,
                   fDump);
            fclose(fDump);
          }
        }
    ...
    HRESULT hr = D3DXDisassembleShader((DWORD*)pbFunc, FALSE, "", &pDisasm);
    ...
    if (SUCCEEDED(hr) && strlen((const char*)pDisasm->GetBufferPointer())) {
      ...
      char* footer_begins =
          comments_end ? strstr(comments_end + 1, "\n\n") : nullptr;
      ...
      if (!_wcsicmp(wszPrefix, L"ps")) {
        ps_disassembly.emplace(
            crc32, tbf::RenderFix::shader_disasm_s{szDisasm, comments_end + 1,
                                                   footer_begins + 1});
      }

      else {
        vs_disassembly.emplace(
            crc32, tbf::RenderFix::shader_disasm_s{szDisasm, comments_end + 1,
                                                   footer_begins + 1});
      }
...
D3D9SetVertexShader_Detour(IDirect3DDevice9* This,
  ...
  if (GetCurrentThreadId() !=
      InterlockedExchangeAdd(&tbf::RenderFix::dwRenderThreadID, 0))
    return D3D9SetVertexShader_Original(This, pShader);
...
bool TBF_ShouldSkipRenderPass(D3DPRIMITIVETYPE PrimitiveType,
                              UINT PrimitiveCount, UINT StartVertex) {
  ...
  const bool tracked_vb = {vb_stream0 ==
                           tbf::RenderFix::tracked_vb.vertex_buffer};
  ...
  if (tracked_vs) {
    ...
    for (int i = 0; i < 16; i++)
      if (tbf::RenderFix::tracked_vs.current_textures[i] != 0)
        tbf::RenderFix::tracked_vs.used_textures.emplace(
            tbf::RenderFix::tracked_vs.current_textures[i]);

    //
    // TODO: Make generic and move into class -- must pass shader type to
    // function
    //
    for (auto&& it : tbf::RenderFix::tracked_vs.constants) {
      for (auto&& it2 : it.struct_members) {
        if (it2.Override)
          tbf::RenderFix::pDevice->SetVertexShaderConstantF(it2.RegisterIndex,
                                                            it2.Data, 1);
      }

      if (it.Override)
        tbf::RenderFix::pDevice->SetVertexShaderConstantF(it.RegisterIndex,
                                                          it.Data, 1);
    }
  }
  ...
  if (tracked_ps) {
    ...
    for (int i = 0; i < 16; i++)
      if (tbf::RenderFix::tracked_ps.current_textures[i] != 0)
        tbf::RenderFix::tracked_ps.used_textures.emplace(
            tbf::RenderFix::tracked_ps.current_textures[i]);
    ...
    for (auto&& it : tbf::RenderFix::tracked_ps.constants) {
      for (auto&& it2 : it.struct_members) {
        if (it2.Override)
          tbf::RenderFix::pDevice->SetPixelShaderConstantF(it2.RegisterIndex,
                                                           it2.Data, 1);
      }

      if (it.Override)
        tbf::RenderFix::pDevice->SetPixelShaderConstantF(it.RegisterIndex,
                                                         it.Data, 1);
    }
  }
  ...
  if (ps_checksum == tbf::RenderFix::tracked_ps.crc32 &&
      tbf::RenderFix::tracked_ps.clamp_coords)
    clamp = true;

  if (vs_checksum == tbf::RenderFix::tracked_vs.crc32 &&
      tbf::RenderFix::tracked_vs.clamp_coords)
    clamp = true;

  if (config.textures.keep_ui_sharp && ps_checksum == 0x17c397fb)
    sharpen = true;
  ...
  if (clamp ||
    ...
    D3D9SetSamplerState_Original(tbf::RenderFix::pDevice, 0, D3DSAMP_ADDRESSU,
                                 D3DTADDRESS_CLAMP);
    D3D9SetSamplerState_Original(tbf::RenderFix::pDevice, 0, D3DSAMP_ADDRESSV,
                                 D3DTADDRESS_CLAMP);
    D3D9SetSamplerState_Original(tbf::RenderFix::pDevice, 0, D3DSAMP_ADDRESSW,
                                 D3DTADDRESS_CLAMP);
  }

  if (sharpen) {
    float fMin = -3.0f;
    D3D9SetSamplerState_Original(tbf::RenderFix::pDevice, 0,
                                 D3DSAMP_MIPMAPLODBIAS,
                                 *reinterpret_cast<DWORD*>(&fMin));
  }

  bool wireframe = false;

  if (tbf::RenderFix::tracked_vb.wireframes.count(vb_stream0)) wireframe = true;

  if (tracked_vb) {
    tbf::RenderFix::tracked_vb.use();

    tbf::RenderFix::tracked_vb.instances = tbf::RenderFix::draw_state.instances;
    tbf::RenderFix::tracked_vb.instanced +=
        tbf::RenderFix::draw_state.instances;
    tbf::RenderFix::tracked_vb.num_draws++;

    if (tbf::RenderFix::tracked_vb.wireframe) wireframe = true;
  }

  if (tracked_vb && tbf::RenderFix::tracked_vb.cancel_draws) return true;

  // Do these sparate so that we can accurately count used textures even on
  // cancelled passes.
  if (tracked_vs && tbf::RenderFix::tracked_vs.cancel_draws) return true;

  if (tracked_ps && tbf::RenderFix::tracked_ps.cancel_draws) return true;

  if (config.fun_stuff.hollow_eye_mode &&
      vs_checksum == config.fun_stuff.hollow_eye_vs_crc32)
    return true;

  if (config.fun_stuff.disable_pause_dim &&
      ps_checksum == config.fun_stuff.pause_dim_ps_crc32)
    return true;

  if (config.fun_stuff.disable_smoke &&
      ps_checksum == config.fun_stuff.smoke_ps_crc32 &&
      tbf::RenderFix::draw_state.current_tex[0] == 0x16f618a8)
    return true;

  if ((!tbf::RenderFix::draw_state.fullscreen_blit) &&
      config.render.aspect_correction && needs_aspect > 0) {
    bool skip = false;
    ...
    if ((!skip) &&
        (tbf::RenderFix::aspect_ratio_data.whitelist.vertex_shaders.count(
             vs_checksum) ||
         tbf::RenderFix::aspect_ratio_data.whitelist.pixel_shaders.count(
             ps_checksum) ||
         tbf::RenderFix::aspect_ratio_data.whitelist.textures.count(
             tbf::RenderFix::draw_state.current_tex[0]))) {
      extern void TBF_Viewport_HUD(IDirect3DDevice9 * This,
                                   uint32_t vs_checksum, uint32_t ps_checksum);

      {
        // Only correct this shader at the title screen / in menus
        bool skip = (vs_checksum == 0x1982d008) &&
                    (!(*TBF_GetFlagFromIdx(7) || *TBF_GetFlagFromIdx(13)));
  ...
  if (config.render.validation) {
    ...
    if (hr != S_OK) {
      ...
      dll_log->Log(L"[D3D9Valid]  Current vs: %x, Current ps: %x", vs_checksum,
                   ps_checksum);
    }
  }
...
void tbf::RenderFix::ClearBlackBars(void) {
  if (!*TBF_GetFlagFromIdx(31)) return;
  ...
  if (height != tbf::RenderFix::height) {
    ...
    D3D9SetRenderState(tbf::RenderFix::pDevice, D3DRS_SCISSORTESTENABLE,
                       tbf::RenderFix::draw_state.scissor_test);
  }
...
D3D9EndScene_Detour(IDirect3DDevice9* This) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice) return D3D9EndScene(This);
  ...
  if (scene_count++ > 0) return D3D9EndScene(This);

  if (pending_loads()) TBFix_LoadQueuedTextures();

  if (config.render.clear_blackbars) tbf::RenderFix::ClearBlackBars();

  tbf::RenderFix::draw_state.cegui_active = true;

  static HMODULE hMod = GetModuleHandle(config.system.injector.c_str());
  ...
  if (config.render.osd_disclaimer) {
    SKX_DrawExternalOSD(
        ...
        "   * This message will go away the first time you actually read it "
        "and successfully toggle the OSD.");
  }

  else {
    static SK_ResizeOSD_pfn SK_ResizeOSD =
        TBF_ImportFunctionFromSpecialK("SK_ResizeOSD", SK_ResizeOSD);

    extern bool __show_cache;
    extern DWORD last_queue_update;

    if (last_queue_update + 250 < timeGetTime()) mod_text = "";
  ...
  memset(tbf::RenderFix::draw_state.viewport_off, static_cast<int>(0.0f),
         sizeof(float) * 4);
  memset(tbf::RenderFix::draw_state.mat4_0, static_cast<int>(0.0f),
         sizeof(float) * 16);
...
COM_DECLSPEC_NOTHROW
void STDMETHODCALLTYPE D3D9EndFrame_Pre(void) {
  if (GetCurrentThreadId() !=
      InterlockedExchangeAdd(&tbf::RenderFix::dwRenderThreadID, 0))
...
D3D9EndFrame_Post(HRESULT hr, IUnknown* device) {
  // Ignore anything that's not the primary render device.
  if (device != tbf::RenderFix::pDevice) return SK_EndBufferSwap(hr, device);
  ...
  if (*TBF_GetFlagFromIdx(7))
    original_aspect =
        (float)tbf::RenderFix::width / (float)tbf::RenderFix::height;
...
D3D9UpdateSurface_Detour(IDirect3DDevice9* This,
  ...
  HRESULT hr =
      D3D9UpdateSurface_Original(This, pSourceSurface, pSourceRect,
  ...
  if (SUCCEEDED(hr)) {
    ...
    HRESULT hr2 = pDestinationSurface->GetContainer(__uuidof(IDirect3DTexture9),
                                                    (void**)&pBase);

    if (SUCCEEDED(hr2) && pBase != nullptr) {
      if (D3DXSaveTextureToFile == nullptr) {
        D3DXSaveTextureToFile = (D3DXSaveTextureToFile_t)GetProcAddress(
            tbf::RenderFix::d3dx9_43_dll, "D3DXSaveTextureToFileW");
      }

      if (D3DXSaveTextureToFile != nullptr) {
        wchar_t wszFileName[MAX_PATH] = {L'\0'};
        _swprintf(wszFileName, L"textures\\UpdateSurface_%x.png", pBase);
...
HRESULT
STDMETHODCALLTYPE
D3D9SetScissorRect_Detour(IDirect3DDevice9* This, const RECT* pRect) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice)
  ...
  // If we don't care about aspect ratio or if we're in a skit, then just
  // early-out
  if ((!config.render.aspect_correction) ||
      ((!tbf::RenderFix::draw_state.fix_scissor) || *TBF_GetFlagFromIdx(34)))
    return D3D9SetScissorRect_Original(This, pRect);
  ...
  if (config.render.aspect_ratio > 1.7777f) {
    float left_ndc =
        2.0f * ((float)pRect->left / (float)tbf::RenderFix::width) - 1.0f;
    float right_ndc =
        2.0f * ((float)pRect->right / (float)tbf::RenderFix::width) - 1.0f;
    ...
    float top_ndc =
        2.0f * ((float)pRect->top / (float)tbf::RenderFix::height) - 1.0f;
    float bottom_ndc =
        2.0f * ((float)pRect->bottom / (float)tbf::RenderFix::height) - 1.0f;

    int height = (int)((9.0f / 16.0f) * tbf::RenderFix::width);

    fixed_scissor.top = (LONG)((top_ndc * height + height) / 2.0f + y_off);
    fixed_scissor.bottom =
        (LONG)((bottom_ndc * height + height) / 2.0f + y_off);
  }
...
HRESULT
STDMETHODCALLTYPE
D3D9SetViewport_Detour(IDirect3DDevice9* This, CONST D3DVIEWPORT9* pViewport) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice)
    return D3D9SetViewport_Original(This, pViewport);

  auto PostProcessMipmaps = [pViewport](void) -> void {
    if (config.render.force_post_mips) {
      ...
      if (SUCCEEDED(tbf::RenderFix::pDevice->GetTexture(0, &pBaseTex0))) {
        ...
        tbf::RenderFix::pDevice->SetSamplerState(0, D3DSAMP_MAGFILTER,
                                                 D3DTEXF_LINEAR);
        tbf::RenderFix::pDevice->SetSamplerState(0, D3DSAMP_MINFILTER,
                                                 D3DTEXF_LINEAR);
        tbf::RenderFix::pDevice->SetSamplerState(0, D3DSAMP_MIPFILTER,
                                                 D3DTEXF_POINT);
      }
      ...
      if (SUCCEEDED(tbf::RenderFix::pDevice->GetTexture(1, &pBaseTex1))) {
        ...
        tbf::RenderFix::pDevice->SetSamplerState(1, D3DSAMP_MAGFILTER,
                                                 D3DTEXF_LINEAR);
        tbf::RenderFix::pDevice->SetSamplerState(1, D3DSAMP_MINFILTER,
                                                 D3DTEXF_LINEAR);
        tbf::RenderFix::pDevice->SetSamplerState(1, D3DSAMP_MIPFILTER,
                                                 D3DTEXF_POINT);
      }
    }
  };
  ...
  //
  //
  if ((pViewport->Width == tbf::RenderFix::width / 2 &&
       pViewport->Height == tbf::RenderFix::height / 2)) {
    PostProcessMipmaps();
  ...
  //
  //
  if ((pViewport->Width == tbf::RenderFix::width / 4 &&
       pViewport->Height == tbf::RenderFix::height / 4)) {
    PostProcessMipmaps();
  ...
  //
  //
  if ((pViewport->Width == tbf::RenderFix::width / 3 &&
       pViewport->Height == tbf::RenderFix::height / 3) ||
      (pViewport->Width == tbf::RenderFix::width / 5 &&
       pViewport->Height == tbf::RenderFix::height / 5) ||
      (pViewport->Width == tbf::RenderFix::width / 6 &&
       pViewport->Height == tbf::RenderFix::height / 6) ||
      (pViewport->Width == tbf::RenderFix::width / 7 &&
       pViewport->Height == tbf::RenderFix::height / 7) ||
      (pViewport->Width == tbf::RenderFix::width / 8 &&
       pViewport->Height == tbf::RenderFix::height / 8) ||
      (pViewport->Width == tbf::RenderFix::width / 9 &&
       pViewport->Height == tbf::RenderFix::height / 9) ||
      (pViewport->Width == tbf::RenderFix::width / 10 &&
       pViewport->Height == tbf::RenderFix::height / 10)) {
    PostProcessMipmaps();
  ...
  //
  if (pViewport->Width == pViewport->Height &&
      (pViewport->Width == 512 || pViewport->Width == 1024 ||
       pViewport->Width == 2048)) {
    // tex_log->Log (L"[Shadow Mgr] (Env. Resolution: (%lu x %lu)",
  ...
  //
            ...
            pViewport->Height ==
                (TBF_NextPowerOfTwo(tbf::RenderFix::height / 2) >> 1))) {
    PostProcessMipmaps();
    ...
    if (config.render.postproc_ratio > 0.0f) {
      ...
      rescaled_post_proc.Width =
          (DWORD)(tbf::RenderFix::width * config.render.postproc_ratio *
                  scale_x);
      rescaled_post_proc.Height =
          (DWORD)(tbf::RenderFix::height * config.render.postproc_ratio *
                  scale_y);
...
void TBF_Viewport_HUD(IDirect3DDevice9* This, uint32_t vs_checksum,
                      uint32_t ps_checksum) {
  // If non-1.0, the draw is in projective-space... if 1.0 it is a regular
  // HUD/UI draw
  if (tbf::RenderFix::draw_state.mat4_0[15] != 1.0f) return;

  // These should never be aspect ratio corrected

  if (tbf::RenderFix::aspect_ratio_data.blacklist.vertex_shaders.count(
          vs_checksum) ||
      tbf::RenderFix::aspect_ratio_data.blacklist.pixel_shaders.count(
          ps_checksum) ||
      tbf::RenderFix::aspect_ratio_data.blacklist.textures.count(
          tbf::RenderFix::draw_state.current_tex[0])) {
    // UI element (the only one) that shares a shader with worldspace HUD
    // elements
  ...
  // For proper clipping behavior on the map screen, skip aspect ratio
  // correction only
  //   when PS 0xdb409773 (map) is not active.
  //
  //   This produces streaming particles on the menu screens that fill the
  //   entire screen
  if (tbf::RenderFix::draw_state.current_tex[0] == 0x620950f9 &&
      ps_checksum != 0xdb409773)
    return;
  ...
  if (tbf::RenderFix::width > tbf::RenderFix::height * (16.0f / 9.0f)) {
    ...
    new_vp.X =
        (tbf::RenderFix::width - tbf::RenderFix::height * (16.0f / 9.0f)) / 2;
    new_vp.Y = 0;
    ...
    new_vp.X = 0;
    new_vp.Y =
        (tbf::RenderFix::height - tbf::RenderFix::width * (9.0f / 16.0f)) / 2;
    D3D9SetViewport_Original(This, &new_vp);
  }
}
void TBF_AdjustViewport(IDirect3DDevice9* This, bool UI) {
  ...
  DWORD height =
      static_cast<DWORD>((9.0f / 16.0f) * static_cast<float>(vp9_orig.Width));
  ...
  if (height != vp9_orig.Height) {
    D3DVIEWPORT9 vp9;
    vp9.X = vp9_orig.X;
    vp9.Y = vp9_orig.Y + (vp9_orig.Height - height) / 2;
    vp9.Width = width;
    vp9.Height = height;
    vp9.MinZ = vp9_orig.MinZ;
    vp9.MaxZ = vp9_orig.MaxZ;
  ...
  if (width != vp9_orig.Width) {
    D3DVIEWPORT9 vp9;
    vp9.X = vp9_orig.X + (vp9_orig.Width - width) / 2;
    vp9.Y = vp9_orig.Y;
    vp9.Width = width;
    vp9.Height = height;
    vp9.MinZ = vp9_orig.MinZ;
...
HRESULT
STDMETHODCALLTYPE
D3D9DrawIndexedPrimitive_Detour(IDirect3DDevice9* This, D3DPRIMITIVETYPE Type,
                                INT BaseVertexIndex, UINT MinVertexIndex,
                                UINT NumVertices, UINT startIndex,
                                UINT primCount) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice) {
    return D3D9DrawIndexedPrimitive_Original(This, Type, BaseVertexIndex,
  ...
  HRESULT hr = D3D9DrawIndexedPrimitive_Original(This, Type, BaseVertexIndex,
                                                 MinVertexIndex, NumVertices,
                                                 startIndex, primCount);
...
HRESULT
STDMETHODCALLTYPE
D3D9SetVertexShaderConstantF_Detour(IDirect3DDevice9* This, UINT StartRegister,
                                    CONST float* pConstantData,
                                    UINT Vector4fCount) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice) {
    return D3D9SetVertexShaderConstantF_Original(This, StartRegister,
                                                 pConstantData, Vector4fCount);
  }

  if (StartRegister == 240 && Vector4fCount >= 1)
    memcpy(tbf::RenderFix::draw_state.viewport_off, pConstantData,
           sizeof(float) * 4);
  ...
  if (StartRegister == 240 && Vector4fCount == 1 &&
    ...
    if (pConstantData[0] == -1.0f / 64.0f) {
      dim = 64UL;
      // dll_log->Log (L" 64x64 Shadow: VS CRC: %lu, PS CRC: %lu", vs_checksum,
      // ps_checksum);
    }

    else if (pConstantData[0] == -1.0f / 128.0f) {
      dim = 128UL;
      // dll_log->Log (L" 128x128 Shadow: VS CRC: %lu, PS CRC: %lu",
      // vs_checksum, ps_checksum);
    }

    else if (pConstantData[0] == -1.0f / 256.0f) {
      dim = 256UL;
      // dll_log->Log (L" 256x256 Shadow: VS CRC: %lu, PS CRC: %lu",
      // vs_checksum, ps_checksum);
    }
    ...
    if (pConstantData[2] != 0.0f || pConstantData[3] != 0.0f) {
      dll_log->Log(L"[   D3D9   ] Assertion failed: non-zero 2 or 3 (line %lu)",
                   __LINE__);
    }
  ...
  // Post-Processing
  //
  else if (StartRegister == 240 && Vector4fCount == 1 &&
           (pConstantData[0] ==
                -1.0f / (float)TBF_NextPowerOfTwo(tbf::RenderFix::width / 2) &&
            pConstantData[1] ==
                1.0f / (float)(TBF_NextPowerOfTwo(tbf::RenderFix::height / 2) >>
                               1)) &&
           config.render.postproc_ratio > 0.0f) {
    if (SUCCEEDED(
            This->GetRenderTarget(0, &tbf::RenderFix::pPostProcessSurface)))
    ...
    newData[0] = -1.0f / ((float)tbf::RenderFix::width *
                          config.render.postproc_ratio * scale_x);
    newData[1] = 1.0f / ((float)tbf::RenderFix::height *
                         config.render.postproc_ratio * scale_y);
  ...
  if (StartRegister == 240 && Vector4fCount == 1 &&
    ...
    if (pConstantData[0] == -1.0f / 512.0f) {
      dim = 512UL;
      // dll_log->Log (L" 512x512 Shadow: VS CRC: %lu, PS CRC: %lu",
      // vs_checksum, ps_checksum);
    }

    else if (pConstantData[0] == -1.0f / 1024.0f) {
      dim = 1024UL;
      // dll_log->Log (L" 1024x1024 Shadow: VS CRC: %lu, PS CRC: %lu",
      // vs_checksum, ps_checksum);
    }

    else if (pConstantData[0] == -1.0f / 2048.0f) {
      dim = 2048UL;
      // dll_log->Log (L" 2048x2048 Shadow: VS CRC: %lu, PS CRC: %lu",
      // vs_checksum, ps_checksum);
    }
  ...
  if (StartRegister == 0 && (Vector4fCount == 2 || Vector4fCount == 3)) {
    ...
    if (This == tbf::RenderFix::pDevice &&
        SUCCEEDED(This->GetRenderTarget(0, &pSurf)) && pSurf != nullptr) {
      ...
      if (config.render.postproc_ratio > 0.0f) {
        if (desc.Width == tbf::RenderFix::width &&
          ...
          if (pSurf == tbf::RenderFix::pPostProcessSurface) {
            ...
            float rescale_x =
                (float)TBF_NextPowerOfTwo(tbf::RenderFix::width / 2) /
                ((float)tbf::RenderFix::width * config.render.postproc_ratio *
                 scale_x);
            float rescale_y =
                (float)(TBF_NextPowerOfTwo(tbf::RenderFix::height / 2) >> 1) /
                ((float)tbf::RenderFix::height * config.render.postproc_ratio *
                 scale_y);
            ...
            return D3D9SetVertexShaderConstantF_Original(This, 0, newData,
                                                         Vector4fCount);
          }
        }
      }
  ...
  if (StartRegister == 0) {
    ...
    if (Vector4fCount == 5) {
      memcpy(tbf::RenderFix::draw_state.mat4_0, pConstantData,
             sizeof(float) * 16);
      ...
      if (pConstantData[0] == 2.0f / 1280.0f &&
        ...
        //
        if ((vs_checksum == 0x66e0873 &&
             (ps_checksum == 3087596655 || ps_checksum == 0x975d2194)) ||
            ps_checksum == 0xf00fa274) {
  ...
  if (config.render.smaa.override_game &&
      (vs_checksum == config.render.smaa.smaa_vs0_crc32 ||
       vs_checksum == config.render.smaa.smaa_vs1_crc32 ||
       vs_checksum == config.render.smaa.smaa_vs2_crc32)) {
    // dll_log->Log (L"[SMAA Log] Crc32: %x, Start Register: %lu, Vec4Count: %lu
    ...
    if (StartRegister == 16 && Vector4fCount == 3) {
      ...
      smaa_constants_s* smaa_constants = (smaa_constants_s*)override_params;

      smaa_constants->general.threshold = config.render.smaa.threshold;
      smaa_constants->general.max_search_steps =
          (float)config.render.smaa.max_search_steps;
      smaa_constants->general.max_search_steps_diag =
          (float)config.render.smaa.max_search_steps_diag;
      smaa_constants->general.corner_rounding =
          config.render.smaa.corner_rounding;

      smaa_constants->predication.threshold =
          config.render.smaa.predication_threshold;
      smaa_constants->predication.scale = config.render.smaa.predication_scale;
      smaa_constants->predication.strength =
          config.render.smaa.predication_strength;

      smaa_constants->reprojection.weight =
          config.render.smaa.reprojection_weight;

      return D3D9SetVertexShaderConstantF_Original(
          This, StartRegister, override_params, Vector4fCount);
    }
  }
...
HRESULT
STDMETHODCALLTYPE
D3D9SetPixelShaderConstantF_Detour(IDirect3DDevice9* This, UINT StartRegister,
                                   CONST float* pConstantData,
                                   UINT Vector4fCount) {
  if (config.render.smaa.override_game &&
      (ps_checksum == config.render.smaa.smaa_ps0_crc32 ||
    ...
    if (StartRegister == 0 && Vector4fCount == 3) {
      ...
      return D3D9SetPixelShaderConstantF_Original(
          This, StartRegister, override_params, Vector4fCount);
    }
  }

  return D3D9SetPixelShaderConstantF_Original(This, StartRegister,
                                              pConstantData, Vector4fCount);
}
...
HRESULT
STDMETHODCALLTYPE
D3D9DrawPrimitive_Detour(IDirect3DDevice9* This, D3DPRIMITIVETYPE PrimitiveType,
                         UINT StartVertex, UINT PrimitiveCount) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice) {
    dll_log->Log(
        L"[Render Fix] >> WARNING: DrawPrimitive came from unknown "
        L"IDirect3DDevice9! << ");

    return D3D9DrawPrimitive_Original(This, PrimitiveType, StartVertex,
                                      PrimitiveCount);
  }
  ...
  HRESULT hr = D3D9DrawPrimitive_Original(This, PrimitiveType, StartVertex,
                                          PrimitiveCount);
...
const wchar_t* SK_D3D9_PrimitiveTypeToStr(D3DPRIMITIVETYPE pt) {
  switch (pt) {
    case D3DPT_POINTLIST:
      return L"D3DPT_POINTLIST";
    case D3DPT_LINELIST:
      return L"D3DPT_LINELIST";
    case D3DPT_LINESTRIP:
      return L"D3DPT_LINESTRIP";
    case D3DPT_TRIANGLELIST:
      return L"D3DPT_TRIANGLELIST";
    case D3DPT_TRIANGLESTRIP:
      return L"D3DPT_TRIANGLESTRIP";
    case D3DPT_TRIANGLEFAN:
      return L"D3DPT_TRIANGLEFAN";
  }
...
D3D9DrawPrimitiveUP_Detour(IDirect3DDevice9* This,
  ...
  HRESULT hr = D3D9DrawPrimitiveUP_Original(This, PrimitiveType, PrimitiveCount,
...
D3D9DrawIndexedPrimitiveUP_Detour(IDirect3DDevice9* This,
  ...
  HRESULT hr = D3D9DrawIndexedPrimitiveUP_Original(
...
void tbf::RenderFix::Reset(IDirect3DDevice9* This,
                           D3DPRESENT_PARAMETERS* pPresentationParameters) {
  static volatile ULONG reset_count = 0UL;
...
COM_DECLSPEC_NOTHROW
HRESULT
__stdcall D3D9Reset_Detour(IDirect3DDevice9* This,
                           D3DPRESENT_PARAMETERS* pPresentationParameters) {
  tbf::RenderFix::Reset(This, pPresentationParameters);

  HRESULT hr = D3D9Reset_Original(This, pPresentationParameters);
...
HRESULT
STDMETHODCALLTYPE
D3D9CreateVertexBuffer_Detour(_In_ IDirect3DDevice9* This, _In_ UINT Length,
                              _In_ DWORD Usage, _In_ DWORD FVF,
                              _In_ D3DPOOL Pool,
                              _Out_ IDirect3DVertexBuffer9** ppVertexBuffer,
                              _In_ HANDLE* pSharedHandle) {
  HRESULT hr = D3D9CreateVertexBuffer_Original(This, Length, Usage, FVF, Pool,
...
HRESULT
STDMETHODCALLTYPE
D3D9SetStreamSource_Detour(IDirect3DDevice9* This, UINT StreamNumber,
                           IDirect3DVertexBuffer9* pStreamData,
                           UINT OffsetInBytes, UINT Stride) {
  // Ignore anything that's not the primary render device.
  if (This != tbf::RenderFix::pDevice) {
    return D3D9SetStreamSource_Original(This, StreamNumber, pStreamData,
                                        OffsetInBytes, Stride);
  }

  HRESULT hr = D3D9SetStreamSource_Original(This, StreamNumber, pStreamData,
                                            OffsetInBytes, Stride);
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (StreamNumber == 0) vb_stream0 = pStreamData;
  }
...
HRESULT
STDMETHODCALLTYPE
D3D9SetStreamSourceFreq_Detour(_In_ IDirect3DDevice9* This,
                               _In_ UINT StreamNumber,
                               _In_ UINT FrequencyParameter) {
  if (StreamNumber == 0 && FrequencyParameter & D3DSTREAMSOURCE_INDEXEDDATA) {
    tbf::RenderFix::draw_state.instances =
        (FrequencyParameter & (~D3DSTREAMSOURCE_INDEXEDDATA));
  }

  if (StreamNumber == 1 && FrequencyParameter & D3DSTREAMSOURCE_INSTANCEDATA) {
  }

  return D3D9SetStreamSourceFreq_Original(This, StreamNumber,
                                          FrequencyParameter);
}

__declspec(noinline)
    D3DPRESENT_PARAMETERS* __stdcall SK_SetPresentParamsD3D9_Detour(
        IDirect3DDevice9* device, D3DPRESENT_PARAMETERS* pparams) {
  if (device != nullptr) {
    dll_log->Log(
        L"[Render Fix] %% Caught D3D9 Swapchain :: Fullscreen=%s "
        L" (%lux%lu @%#3lu Hz) "
        ...
        pparams->FullScreen_RefreshRateInHz, pparams->hDeviceWindow, device);
  }
  ...
  aspect_ratio.setAspectRatio((float)tbf::RenderFix::width /
                              (float)tbf::RenderFix::height);

  if (config.render.aspect_ratio > 16.0f / 9.0f) {
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xb8453aa6);  // Red Horizontal Stripe in Menus
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xc070b309);  // Border around location names [Stretch Horiz]
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0x886664be);  // Skit Background Frame   [ Stretch Horizontally ]
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xcf860df0);  // Skit Background Frame   [ Stretch Horizontally ]
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xd27b7033);  // Expand Horizontally
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xf9aca330);  // Expand Horizontally
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xa650f90b);  // Expand Horizontally
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xc31cc516);  // Expand Horizontally
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xf2a7470b);  // Main UI frame [Stretch Horiz]
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xac8976c1);  // Background for text such as (Favorable Encounter)

    tbf::RenderFix::aspect_ratio_data.blacklist.textures.erase(
        0xb0e5dbd7);  // Stretch Vert
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.erase(
        0x2355f634);  // Stretch Vert
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.erase(
        0x555812e3);  // Stretch Vert
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.erase(
        0x55e94ca3);  // Stretch Vert
  }
    ...
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xc070b309);  // Border around location names [Stretch Vert]
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0xb0e5dbd7);  // Stretch Vert
    tbf::RenderFix::aspect_ratio_data.blacklist.textures.emplace(
        0x2355f634);  // Stretch Vert
...
void tbf::RenderFix::Init(void) {
  ...
  aspect_ratio_data.whitelist.vertex_shaders.emplace(
      0x1982d008);  // Title Screen (Velvet) [Condition: Game State Title
                    // Screen]
  aspect_ratio_data.whitelist.textures.emplace(0x7625bb78);
  aspect_ratio_data.whitelist.textures.emplace(0xb11b9a20);
  aspect_ratio_data.whitelist.textures.emplace(
      0x8ffdd831);  // Trademark Symbol (Title Screen)
  aspect_ratio_data.whitelist.textures.emplace(
      0x7ecb9487);  // Menu Carrot (Colored)
  ...
  aspect_ratio_data.blacklist.textures.emplace(
      0x2d762a65);  // Menu   Background (brown)
  aspect_ratio_data.blacklist.textures.emplace(
      0xee718441);  // Map    Background (dims existing color)
  aspect_ratio_data.blacklist.textures.emplace(
      0x5ce3fac6);  // Framed Background
  aspect_ratio_data.blacklist.textures.emplace(
      0x952b0ff8);  // NPC Dialog Background
  ...
  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9SetSamplerState_Override", D3D9SetSamplerState_Detour,
  ...
  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9SetVertexShader_Override", D3D9SetVertexShader_Detour,
                     (LPVOID*)&D3D9SetVertexShader_Original);

  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9SetPixelShader_Override", D3D9SetPixelShader_Detour,
                     (LPVOID*)&D3D9SetPixelShader_Original);

  // Needed for UI re-scaling
  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9SetScissorRect_Override", D3D9SetScissorRect_Detour,
                     (LPVOID*)&D3D9SetScissorRect_Original);

  TBF_CreateDLLHook2(config.system.injector.c_str(), "D3D9EndScene_Override",
                     D3D9EndScene_Detour, (LPVOID*)&D3D9EndScene);

  TBF_CreateDLLHook2(config.system.injector.c_str(), "D3D9Reset_Override",
                     D3D9Reset_Detour, (LPVOID*)&D3D9Reset_Original);

  user32_dll = LoadLibrary(L"User32.dll");

  TBF_CreateDLLHook2(config.system.injector.c_str(), "SK_BeginBufferSwap",
                     D3D9EndFrame_Pre, (LPVOID*)&SK_BeginBufferSwap);

  TBF_CreateDLLHook2(config.system.injector.c_str(), "SK_EndBufferSwap",
                     D3D9EndFrame_Post, (LPVOID*)&SK_EndBufferSwap);

  TBF_CreateDLLHook2(config.system.injector.c_str(), "SK_SetPresentParamsD3D9",
  ...
  TBF_CreateDLLHook2(
      config.system.injector.c_str(), "D3D9CreateVertexBuffer_Override",
      D3D9CreateVertexBuffer_Detour, (LPVOID*)&D3D9CreateVertexBuffer_Original);

  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9SetStreamSource_Override", D3D9SetStreamSource_Detour,
  ...
  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9DrawPrimitive_Override", D3D9DrawPrimitive_Detour,
  ...
  TBF_CreateDLLHook2(config.system.injector.c_str(),
                     "D3D9DrawPrimitiveUP_Override", D3D9DrawPrimitiveUP_Detour,
  ...
  D3DXDisassembleShader = (D3DXDisassembleShader_pfn)GetProcAddress(
      tbf::RenderFix::d3dx9_43_dll, "D3DXDisassembleShader");
...
void tbf::RenderFix::Shutdown(void) { tex_mgr.Shutdown(); }

tbf::RenderFix::CommandProcessor::CommandProcessor(void) {
  SK_ICommandProcessor& command = *SK_GetCommandProcessor();

  // fovy_         = TBF_CreateVar (SK_IVariable::Float, &config.render.fovy,
  // this); aspect_ratio_ = TBF_CreateVar (SK_IVariable::Float,
  // &config.render.aspect_ratio, this);

  SK_IVariable* aspect_correct_vids =
      TBF_CreateVar(SK_IVariable::Boolean, &config.render.blackbar_videos);
  SK_IVariable* aspect_correction =
      TBF_CreateVar(SK_IVariable::Boolean, &config.render.aspect_correction);

  SK_IVariable* postproc_ratio =
      TBF_CreateVar(SK_IVariable::Float, &config.render.postproc_ratio);
  SK_IVariable* clear_blackbars =
      TBF_CreateVar(SK_IVariable::Boolean, &config.render.clear_blackbars);

  SK_IVariable* remaster_textures =
      TBF_CreateVar(SK_IVariable::Boolean, &config.textures.remaster);
  SK_IVariable* lod_bias =
      TBF_CreateVar(SK_IVariable::Float, &config.textures.lod_bias);
  SK_IVariable* uncompressed =
      TBF_CreateVar(SK_IVariable::Boolean, &config.textures.uncompressed);

  SK_IVariable* rescale_shadows =
      TBF_CreateVar(SK_IVariable::Int, &config.render.shadow_rescale);
  SK_IVariable* rescale_env_shadows =
  ...
  {
    void* addr = TBF_Scan(signature, sizeof(float), nullptr, 8);
    ...
    do {
      ...
      if (GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT |
                             ...
                             (LPCWSTR)addr, &hMod)) {
        if (hMod != GetModuleHandle(nullptr)) continue;
        ...
        aspect_ratio.addrs[aspect_ratio.count++] = (float*)addr;
             ...
             (addr = TBF_ScanEx(signature, sizeof(float), nullptr, addr, 8)));
  }
...
bool tbf::RenderFix::CommandProcessor::OnVarChange(SK_IVariable* var,
                                                   void* val) {
  DWORD dwOld;

  if (var == aspect_ratio_) {
    VirtualProtect((LPVOID)config.render.aspect_addr, 4, PAGE_READWRITE,
                   &dwOld);
    ...
    if (((original < 1.777778f + 0.01f && original > 1.777778f - 0.01f) ||
      ...
      if (val != nullptr)
        dll_log->Log(
            L"[Asp. Ratio]  * Unable to change Aspect Ratio, invalid memory "
            L"address... (%f)",
  ...
  if (var == fovy_) {
    ...
    if (((original < 0.785398f + 0.01f && original > 0.785398f - 0.01f) ||
      ...
      dll_log->Log(L"[Asp. Ratio]  * Changing FOVY from %f to %f", original,
                   config.render.fovy);
      *((float*)config.render.fovy_addr) = config.render.fovy;
    } else {
      if (val != nullptr)
        dll_log->Log(
            L"[Asp. Ratio]  * Unable to change FOVY, invalid memory address... "
            L"(%f)",
...
void tbf::RenderFix::TriggerReset(void) {
  trigger_reset = reset_stage_s::Initiate;
}

extern HMODULE hInjectorDLL;

bool tbf::RenderFix::InstallSGSSAA(void) {
  ((void(__stdcall*)(const wchar_t*))GetProcAddress(
      hInjectorDLL, "SK_NvAPI_SetAppFriendlyName"))(L"Tales of Berseria");
  ((void(__stdcall*)(const wchar_t*))GetProcAddress(
      hInjectorDLL, "SK_NvAPI_SetAppName"))(L"Tales of Berseria.exe");
  ...
  if (config.render.nv.sgssaa_mode == 1) {
    const wchar_t* props[] = {L"CompatibilityBits",
                              wszBits,
                              L"Method",
                              L"2xMSAA",
                              L"ReplayMode",
                              ...
                              nullptr,
                              nullptr};
    return ((BOOL(__stdcall*)(const wchar_t**))GetProcAddress(
        hInjectorDLL, "SK_NvAPI_SetAntiAliasingOverride"))(props);
  }
    ...
    const wchar_t* props[] = {
        L"Method",         L"0x00000000", L"ReplayMode", L"0x00000000",
        L"AutoBiasAdjust", L"On",         L"Override",   L"No",
        nullptr,           nullptr};
    return ((BOOL(__stdcall*)(const wchar_t**))GetProcAddress(
        hInjectorDLL, "SK_NvAPI_SetAntiAliasingOverride"))(props);
  }
...
tbf::RenderFix::reset_state_s tbf::RenderFix::need_reset;

tbf::RenderFix::render_target_tracking_s tbf::RenderFix::tracked_rt;

tbf::RenderFix::shader_tracking_s tbf::RenderFix::tracked_vs;

tbf::RenderFix::shader_tracking_s tbf::RenderFix::tracked_ps;

tbf::RenderFix::vertex_buffer_tracking_s tbf::RenderFix::tracked_vb;

tbf::RenderFix::known_objects_s tbf::RenderFix::known_objs;

void EnumConstant(
    tbf::RenderFix::shader_tracking_s* pShader,
    ID3DXConstantTable* pConstantTable, D3DXHANDLE hConstant,
    tbf::RenderFix::shader_tracking_s::shader_constant_s& constant,
    std::vector<tbf::RenderFix::shader_tracking_s::shader_constant_s>& list) {
  UINT one = 1;

  D3DXCONSTANT_DESC constant_desc;
  if (SUCCEEDED(
          pConstantTable->GetConstantDesc(hConstant, &constant_desc, &one))) {
    ...
    // memcpy (constant.Data, constant_desc.DefaultValue, std::min
    // ((size_t)constant_desc.Bytes, sizeof (float) * 4UL));

    for (UINT j = 0; j < constant_desc.StructMembers; j++) {
      D3DXHANDLE hConstantStruct = pConstantTable->GetConstant(hConstant, j);

      tbf::RenderFix::shader_tracking_s::shader_constant_s struct_constant = {};

      EnumConstant(pShader, pConstantTable, hConstantStruct, struct_constant,
...
void tbf::RenderFix::shader_tracking_s::use(IUnknown* pShader) {
  if (shader_obj != pShader) {
    ...
    static D3DXGetShaderConstantTable_pfn D3DXGetShaderConstantTable =
        (D3DXGetShaderConstantTable_pfn)GetProcAddress(
            d3dx9_43_dll, "D3DXGetShaderConstantTable");

    UINT len;
    if (SUCCEEDED(
            ((IDirect3DVertexShader9*)pShader)->GetFunction(nullptr, &len))) {
      void* pbFunc = malloc(len);

      if (pbFunc != nullptr) {
        if (SUCCEEDED(((IDirect3DVertexShader9*)pShader)
                          ->GetFunction(pbFunc, &len))) {
          CComPtr<ID3DXConstantTable> pConstantTable = nullptr;

          if (SUCCEEDED(D3DXGetShaderConstantTable((DWORD*)pbFunc,
                                                   &pConstantTable))) {
            ...
            if (SUCCEEDED(pConstantTable->GetDesc(&ct_desc))) {
              ...
              for (UINT i = 0; i < constant_count; i++) {
                D3DXHANDLE hConstant = pConstantTable->GetConstant(nullptr, i);

                shader_constant_s constant = {};

                EnumConstant(this, pConstantTable, hConstant, constant,
                             constants);
              }
            }
          }
